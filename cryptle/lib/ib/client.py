...
CONN_STATES = ['DISCONNECTED', 'CONNECTING', 'CONNECTED', 'REDIRECT']


def _need_connection(meth):
...
class EClient(object):
    ...
    def setConnState(self, connState):
        last_state = self.connState
        self.connState = connState
        logger.debug(
            "connState: %s -> %s", CONN_STATES[last_state], CONN_STATES[self.connState]
        )

    def sendMsg(self, msg):
        # todo(alan): wrap outbound methods in decorators such that we can first collect the
        # calling variables, then send the message as method's return value, should result
        # in a cleaner and more explicit API
        full_msg = comm.make_msg(msg)
        fname, _ = parent_fn_context()
        logger.info("-- CLIENT -- %s()", fname)
    ...
    def connect(self, host, port, clientId):
        ...
        try:
            ...
            logger.debug(
                "Connecting to %s:%d with id:%d", self.host, self.port, self.clientId
            )
            ...
            logger.debug("-- TWS -- Version:%d time:%s", server_version, conn_time)
            ...
            logger.error("Connect refused. TWS server inactive")
            raise e
    ...
    def placeOrder(self, orderId: OrderId, contract: Contract, order: Order):
        ...
        flds = []
        flds += [make_field(OUT.PLACE_ORDER), make_field(VERSION), make_field(orderId)]
    ...
    def cancelMktDepth(self, reqId: TickerId):
        ...
        msg = make_field(OUT.CANCEL_MKT_DEPTH) + make_field(VERSION) + make_field(reqId)
    ...
    def reqScannerSubscription(
        ...
        flds = []
        flds += [
            make_field(OUT.REQ_SCANNER_SUBSCRIPTION),
            make_field(VERSION),
            make_field(reqId),
