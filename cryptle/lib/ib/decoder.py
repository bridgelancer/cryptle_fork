...
class HandleInfo(Object):
    """Functor for TWS callback invocations."""

    def __init__(self, wrap=None, proc=None):
        if wrap is None and proc is None:
            raise ValueError("wrap and proc can't both be None")
        ...
        if wrap:
            ...
            self.wrap = wrap
            self.wrapParams = sig.parameters

        if proc:
            self.proc = proc

    def __call__(self, decoder, fields):
        if self.wrap is not None:
            # pass decoder instance to allow access to wrapper instance
            self._interpretWithSignature(decoder, fields)
        elif self.proc is not None:
            # pass decoder instance as well
            self.proc(decoder, iter(fields))

    def __str__(self):
        if self.wrap:
            return "wrap:%s meth:%s" % (self.wrap.__qualname__, self.wrapParams)
        elif self.proc:
            return "prms:%s" % (self.proc)

    def _interpretWithSignature(self, decoder, fields):
        if self.wrapParams is None:
            logger.debug("%s: no param info in", self.wrap)
            return

        nIgnoreFields = 2  # bypass msgId and versionId faster this way
        if len(fields) - nIgnoreFields != len(self.wrapParams) - 1:
            logger.error(
                "diff len fields and params %d %d for fields: %s and handler: %s",
                len(fields),
                len(self.wrapParams),
                fields,
                self.wrap,
        ...
        args = []
        arg_fields = fields[nIgnoreFields:]
        # skip the self param
        wrap_params = tuple(self.wrapParams.items())[1:]
        for ((pname, param), field) in zip(wrap_params, arg_fields):
            if pname != "self":
                try:
                    arg = field.decode('UTF-8')
                except UnicodeDecodeError:
                    arg = field.decode('latin-1')
        ...
        # get the actual callback interface(possibly overriden) from wrapper instance
        method = getattr(decoder.wrapper, self.wrap.__name__)
        logger.debug("calling %s with %s", method, args)
        method(*args)
...
class Decoder(Object):
    ...
    def processOpenOrder(self, fields):
        ...
        order.deltaNeutralOrderType = decode(
            str, fields
        )  # ver 11 field (had a hack for ver 11)
        ...
        if version >= 20:
            ...
            order.scaleInitLevelSize = decode(
                int, fields, SHOW_UNSET
            )  # scaleComponectSize
    ...
    def interpret(self, fields):
        ...
        # Get proper handler from server's message ID
        try:
            msg_id = int(fields[0])
            handler = MSGID_HANDLER_MAP[msg_id]
        except KeyError:
            logger.warning("No handler for message: %d", msg_id)
            return
...
MSGID_HANDLER_MAP = {
    IN.TICK_PRICE: HandleInfo(proc=Decoder.processTickPriceMsg),
    IN.TICK_SIZE: HandleInfo(wrap=EWrapper.tickSize),
    IN.ORDER_STATUS: HandleInfo(proc=Decoder.processOrderStatusMsg),
    IN.ERR_MSG: HandleInfo(wrap=EWrapper.error),
    IN.OPEN_ORDER: HandleInfo(proc=Decoder.processOpenOrder),
    IN.ACCT_VALUE: HandleInfo(wrap=EWrapper.updateAccountValue),
    IN.PORTFOLIO_VALUE: HandleInfo(proc=Decoder.processPortfolioValueMsg),
    IN.ACCT_UPDATE_TIME: HandleInfo(wrap=EWrapper.updateAccountTime),
    IN.NEXT_VALID_ID: HandleInfo(wrap=EWrapper.nextValidId),
    IN.CONTRACT_DATA: HandleInfo(proc=Decoder.processContractDataMsg),
    IN.EXECUTION_DATA: HandleInfo(proc=Decoder.processExecutionDataMsg),
    ...
    IN.HISTORICAL_DATA: HandleInfo(proc=Decoder.processHistoricalDataMsg),
    IN.HISTORICAL_DATA_UPDATE: HandleInfo(proc=Decoder.processHistoricalDataUpdateMsg),
    IN.BOND_CONTRACT_DATA: HandleInfo(proc=Decoder.processBondContractDataMsg),
    IN.SCANNER_PARAMETERS: HandleInfo(wrap=EWrapper.scannerParameters),
    IN.SCANNER_DATA: HandleInfo(proc=Decoder.processScannerDataMsg),
    IN.TICK_OPTION_COMPUTATION: HandleInfo(
        proc=Decoder.processTickOptionComputationMsg
    ),
    ...
    IN.REAL_TIME_BARS: HandleInfo(proc=Decoder.processRealTimeBarMsg),
    ...
    IN.DELTA_NEUTRAL_VALIDATION: HandleInfo(
        proc=Decoder.processDeltaNeutralValidationMsg
    ),
    IN.TICK_SNAPSHOT_END: HandleInfo(wrap=EWrapper.tickSnapshotEnd),
    IN.MARKET_DATA_TYPE: HandleInfo(wrap=EWrapper.marketDataType),
    IN.COMMISSION_REPORT: HandleInfo(proc=Decoder.processCommissionReportMsg),
    IN.POSITION_DATA: HandleInfo(proc=Decoder.processPositionDataMsg),
    ...
    IN.POSITION_MULTI: HandleInfo(proc=Decoder.processPositionMultiMsg),
    ...
    IN.SECURITY_DEFINITION_OPTION_PARAMETER: HandleInfo(
        proc=Decoder.processSecurityDefinitionOptionParameterMsg
    ),
    IN.SECURITY_DEFINITION_OPTION_PARAMETER_END: HandleInfo(
        proc=Decoder.processSecurityDefinitionOptionParameterEndMsg
    ),
    IN.SOFT_DOLLAR_TIERS: HandleInfo(proc=Decoder.processSoftDollarTiersMsg),
    IN.FAMILY_CODES: HandleInfo(proc=Decoder.processFamilyCodesMsg),
    IN.SYMBOL_SAMPLES: HandleInfo(proc=Decoder.processSymbolSamplesMsg),
    IN.SMART_COMPONENTS: HandleInfo(proc=Decoder.processSmartComponents),
    IN.TICK_REQ_PARAMS: HandleInfo(proc=Decoder.processTickReqParams),
    IN.MKT_DEPTH_EXCHANGES: HandleInfo(proc=Decoder.processMktDepthExchanges),
    IN.HEAD_TIMESTAMP: HandleInfo(proc=Decoder.processHeadTimestamp),
    IN.TICK_NEWS: HandleInfo(proc=Decoder.processTickNews),
    IN.NEWS_PROVIDERS: HandleInfo(proc=Decoder.processNewsProviders),
    IN.NEWS_ARTICLE: HandleInfo(proc=Decoder.processNewsArticle),
    IN.HISTORICAL_NEWS: HandleInfo(proc=Decoder.processHistoricalNews),
    IN.HISTORICAL_NEWS_END: HandleInfo(proc=Decoder.processHistoricalNewsEnd),
    IN.HISTOGRAM_DATA: HandleInfo(proc=Decoder.processHistogramData),
    IN.REROUTE_MKT_DATA_REQ: HandleInfo(proc=Decoder.processRerouteMktDataReq),
    IN.REROUTE_MKT_DEPTH_REQ: HandleInfo(proc=Decoder.processRerouteMktDepthReq),
    IN.MARKET_RULE: HandleInfo(proc=Decoder.processMarketRuleMsg),
    IN.PNL: HandleInfo(proc=Decoder.processPnLMsg),
    IN.PNL_SINGLE: HandleInfo(proc=Decoder.processPnLSingleMsg),
    IN.HISTORICAL_TICKS: HandleInfo(proc=Decoder.processHistoricalTicks),
    IN.HISTORICAL_TICKS_BID_ASK: HandleInfo(proc=Decoder.processHistoricalTicksBidAsk),
    IN.HISTORICAL_TICKS_LAST: HandleInfo(proc=Decoder.processHistoricalTicksLast),
    IN.TICK_BY_TICK: HandleInfo(proc=Decoder.processTickByTickMsg),
}
