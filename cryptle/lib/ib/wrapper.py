...
class EWrapper:
    def logAnswer(self):
        """Logs callback invocation by the TWS server."""
        fname, local = parent_fn_context()
        logger.info("-- TWS -- %s", fname)
        logger.debug("-- TWS -- %s(%s)", fname, local)

    def error(self, reqId: TickerId, errorCode: int, errorString: str):
        """Logs erros with the TWS communication.
