...
class Registry:
    ...
    @on('open') # 'open', 'close' event should be generated by aggregator
            ...
            def check(key, whenexec):
                if all(self.lookup_check[constraint] for constraint in whenexec) and \
                   self.logic_status[key] == []: # now only works if no constraint is placed
                       self.generateExecute(key)
            check(key, setup[key][0])
