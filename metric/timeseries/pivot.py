...
class PivotPoints(Timeseries):
    def __init__(self, timestamp, interval, high, low, close, days=1, n=8, list=False):
        ...
        self._days = days
        self._lookback = int((days*24*60*60) / interval) #the number of bars required to be cached
        self._cache    = []
    ...
    def evaluate(self):
        if self.timestamp % (24*60*60) == 86400 - 3600:
            ...
            for i in range(2, self.n + 1):
                self.r[i] = self.period_high + (i-1) * (self.period_high - self.period_low)
                self.s[i] = self.period_low - (i-1) * (self.period_high - self.period_low)
