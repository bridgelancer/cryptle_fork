...
def test_n_per_period_n_per_signal():
    ...
    def twokprice():
        ...
        print("TRIGGERED",
                    datetime.datetime.fromtimestamp(int(aggregator.last_bar_timestamp)).strftime('%Y-%m-%d %H:%M:%S'), stick.c, "\n")
        emitPrintTrigger() # should pass its name (action name) to emitTrigger
    setup      = {'sma':       [aboveSMA, ['open'], [['once per bar'], {}], 1],
                  'twokprice': [twokprice, ['open'], [['once per bar'], {'n per period': [1, 2], 'n per signal': ['sma', 3]}], 2]}
    registry   = Registry(setup)
    bus.bind(registry)
    sma        = SMA(stick.o, 5)
